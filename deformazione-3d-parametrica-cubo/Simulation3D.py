...
class Simulation3D():
    ...
    def _saveInputDataToFile(self):
        ...
        inputData = { "index"                 : self.index,
                      "cube_speed_x"        : self.circle_speed_x,
                      "cube_speed_y"        : self.circle_speed_y,
                      "cube_speed_z"        : self.circle_speed_z,
                      "cube_speed"          : self.circle_speed,
                      "cube_impact_angle_x" : self.circle_impact_angle_x,
                      "cube_impact_angle_y" : self.circle_impact_angle_y,
                      "cube_rotation_x"     : self.cube_rotation_x,
                      "cube_rotation_y"     : self.cube_rotation_y,
                      "cube_rotation_z"     : self.cube_rotation_z,
                      "cube_radius"         : self.circle_radius }
    ...
    def runSimulation( self,
        ...
        self.cube_rotation_x = CUBE_ROTATION_X
        self.cube_rotation_y = CUBE_ROTATION_Y
        self.cube_rotation_z = CUBE_ROTATION_Z
        ...
        #----------- crea PARTI e SET: cubo -----------#

        # crea sketch cubo, quadrato con lato 2*radius
        sketch_cube = model.ConstrainedSketch( name      = 'sketch-cube', 
                                                 sheetSize = 2*self.CIRCLE_BASE_RADIUS )
                                                 
                                                 
        sketch_cube.rectangle( ( -self.CIRCLE_BASE_RADIUS, -self.CIRCLE_BASE_RADIUS ), 
                                (  self.CIRCLE_BASE_RADIUS, self.CIRCLE_BASE_RADIUS ) )
                                 

        # crea plate usando lo sketch
        part_cube = model.Part( name           = 'cube', 
                                 dimensionality = abaqusConstants.THREE_D, 
                                 type           = abaqusConstants.DEFORMABLE_BODY )
                                                     
        part_cube.BaseSolidExtrude(sketch = sketch_cube, depth=2*self.CIRCLE_BASE_RADIUS)


        # set con tutta la lastra, per il materiale
        part_cube.Set( name  = 'set-all', 
                        cells = part_cube.cells )
        

        # superficie per l'interaction
        part_cube.Surface( name = 'surface-all', side1Faces = part_cube.faces)

        # set tutte  le superfici esterni, per l'output
        part_cube.Set( name = 'surface-all', faces = part_cube.faces )
        ...
        material_cube = model.Material( name = 'material-cube' )
        material_cube.Density( table = self.lead_density )
        material_cube.Elastic( table = self.lead_elastic )
        ...
        # CREA SEZIONE PER IL CUBO E ASSEGNACI IL CUBO
        model.HomogeneousSolidSection( name      = 'section-cube', 
                                       material  = 'material-cube', 
                                       thickness = None )

        # ASSEGNA MATERIALE AL CUBO       
        part_cube.SectionAssignment( region      = part_cube.sets['set-all'], 
                                     sectionName = 'section-cube' )
        ...
        # i solidi fatti per estrazione non sono centrati rispetto al piano XY, quindi devo sottrarre meta' della profondita'
        # alla z per centrarli rispetto alla posizione desiderata
        instance_cube = model.rootAssembly.Instance( name      = 'cube', 
                                     part      = part_cube, 
                                     dependent = abaqusConstants.ON)
        
        instance_cube.translate(( self.circle_origin_x, self.circle_origin_y, self.circle_origin_z - self.CIRCLE_BASE_RADIUS ))
        
        # ruota cubo
        instance_cube.rotateAboutAxis(
            axisPoint = (self.circle_origin_x, self.circle_origin_y, self.circle_origin_z),
            axisDirection = (1, 0, 0),      # asse X
            angle = CUBE_ROTATION_X
        )
        instance_cube.rotateAboutAxis(
            axisPoint = (self.circle_origin_x, self.circle_origin_y, self.circle_origin_z),
            axisDirection = (0, 1, 0),      # asse Y
            angle = CUBE_ROTATION_Y
        )
        instance_cube.rotateAboutAxis(
            axisPoint = (self.circle_origin_x, self.circle_origin_y, self.circle_origin_z),
            axisDirection = (0, 0, 1),      # asse Z
            angle = CUBE_ROTATION_Z
        )
        ...
        # crea reference point nella coordinata dove si trova il centro del cubo
        RP_cube_id     = model.rootAssembly.ReferencePoint( point = ( self.circle_origin_x, self.circle_origin_y, self.circle_origin_z) ).id
        
        RP_cube_region = regionToolset.Region( referencePoints = ( model.rootAssembly.referencePoints[RP_cube_id], ) )
        RP_cube_set    = model.rootAssembly.Set( name = "cube-rp", referencePoints = ( model.rootAssembly.referencePoints[RP_cube_id], ))
        
        
        # assegna rigid body constraint alla palla associato al reference point
        model.RigidBody( name           = 'constraint-cube-rigid-body', 
                         refPointRegion = RP_cube_region, 
                         bodyRegion     = model.rootAssembly.instances['cube'].sets['set-all'] )
        ...
        # aggiungi una history output request per la velocita' verticale del cubo, in modo da potergli applicare un filtro
        ...
        # interazione cubo - lastra
        model.SurfaceToSurfaceContactExp( name                = 'Int-Cube-Plate', 
                                          createStepName      = 'Initial', 
                                          main                = model.rootAssembly.instances['cube'].surfaces['surface-all'],
        ...
        part_cube.seedPart( size = self.circle_seed_size )

        # questa parte si puo' meshare solo se gli elementi hanno una forma tetraedrica, e gli va detto esplicitamente
        part_cube.setMeshControls(regions = part_cube.cells, elemShape = abaqusConstants.TET)

        part_cube.generateMesh()


        # scalo la mesh
        scale = self.circle_radius / self.CIRCLE_BASE_RADIUS

        for n in range(0,len(part_cube.nodes)):
            coords = part_cube.nodes[n].coordinates

            # scale the nodes
            new_coords = [coord * scale for coord in coords]

            # change the nodes of current part
            part_cube.editNode(coordinate1=new_coords[0], coordinate2=new_coords[1],coordinate3=new_coords[2],
                nodes=part_cube.nodes[n])

        '''
        # printa quanti nodi ha il cubo
        log("radius " + str(self.circle_radius))
        log("quantita' nodi palla:" + str(len(part_cube.nodes)))
        '''
        ...
        # Check if simulation has completed, i.e. if the cube has stopped
        cube_region = session.openOdb(JOB_NAME + '.odb').steps['Step-1'].historyRegions['Node ASSEMBLY.1']
        cube_v2Data = cube_region.historyOutputs['V2_FILTER-1'].data
        (simulation_length, cube_final_velocity) = cube_v2Data[-1]

        # Variable that says whether simulation has completed
        simulation_completed = (cube_final_velocity >= 0.0)
        ...
        #outputRegionCube = odb.rootAssembly.instances['CUBE'].nodeSets['SET-ALL']
        outputRegionCubeExternal = odb.rootAssembly.instances['CUBE'].nodeSets['SURFACE-ALL']


        # SAVING CUBE VELOCITY
        if SAVECIRCLEVELOCITY:
            ...
            velocity_output_filename = os.path.join(self.new_path, str(self.index) + '_cube_velocity_y.csv')
        ...
        if SAVECOORDINATES:
            ...
            coordinates_cube_1 = firstFrame.fieldOutputs['COORD'].getSubset(region = outputRegionCubeExternal)
            coordinates_cube_1_df = pd.DataFrame( { 'Id'      : [ values.nodeLabel for values in coordinates_cube_1.values ],
                                                      'X_Coord' : [ values.data[0]   for values in coordinates_cube_1.values ],
                                                      'Y_Coord' : [ values.data[1]   for values in coordinates_cube_1.values ],
                                                      'Z_Coord' : [ values.data[2]   for values in coordinates_cube_1.values ] } )
            coordinate_cube_1_filename = os.path.join(self.new_path, str(self.index) + '_input_coordinates_cube_1.csv')
            coordinates_cube_1_df.to_csv(coordinate_cube_1_filename, index = False)


            # ****** Coordinates 1/30 seconds before impact ******
            if (frameOne30BeforeImpact != None):
                coordinates_cube_2 = frameOne30BeforeImpact.fieldOutputs['COORD'].getSubset( region = outputRegionCubeExternal )                
                coordinates_cube_2_df = pd.DataFrame( { 'Id'    : [ values.nodeLabel for values in coordinates_cube_2.values ],
                                                        'X_Coord' : [ values.data[0]   for values in coordinates_cube_2.values ],
                                                        'Y_Coord' : [ values.data[1]   for values in coordinates_cube_2.values ],
                                                        'Z_Coord' : [ values.data[2]   for values in coordinates_cube_2.values ] } )
                coordinate_cube_2_filename = os.path.join(self.new_path, str(self.index) + '_input_coordinates_cube_2.csv')
                coordinates_cube_2_df.to_csv(coordinate_cube_2_filename, index = False)
...
sim.runSimulation(
    ...
    ALPHA_Y         = alpha_Y,
    ALPHA_X         = alpha_X,
    CUBE_ROTATION_X = rot_x,
    CUBE_ROTATION_Y = rot_y,
    CUBE_ROTATION_Z = rot_z,
    SIMULATION_ID   = idx,
