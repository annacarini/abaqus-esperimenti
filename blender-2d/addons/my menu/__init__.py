...
CIRCLE_VELOCITY_DEFAULT_VALUE = 1000
CIRCLE_VELOCITY_RANGE = [200, 1500]

CIRCLE_ALPHA_PROPERTY_NAME = "alpha"
CIRCLE_ALPHA_DEFAULT_VALUE = 0
CIRCLE_ALPHA_RANGE = [-60, 60]  

CIRCLE_RADIUS_PROPERTY_NAME = "radius"
CIRCLE_RADIUS_DEFAULT_VALUE = 2.5
CIRCLE_RADIUS_RANGE = [1.5, 4]
...
CIRCLE_DEFAULT_POSITION = (trajectory*math.sin(math.radians(CIRCLE_ALPHA_DEFAULT_VALUE)), 0, trajectory*math.cos(math.radians(CIRCLE_ALPHA_DEFAULT_VALUE)))
...
bl_info = {
    ...
    "version": (1, 0),
    "blender": (3, 0, 0),
    "location": "Right 3d View Panel > My menu",
    "category": "Object",
}
...
class SimulationProperties(bpy.types.PropertyGroup):
    ...
    alpha: bpy.props.IntProperty(
        name = CIRCLE_ALPHA_PROPERTY_NAME,
        default = CIRCLE_ALPHA_DEFAULT_VALUE,
        min = CIRCLE_ALPHA_RANGE[0],
        max = CIRCLE_ALPHA_RANGE[1],
        step=1
    )
    ...
    def reset(self):
        ...
        self.alpha = CIRCLE_ALPHA_DEFAULT_VALUE
...
class OT_play_animation(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type == 'TIMER':
            ...
            # muovi cerchio
            self.move_circle(context, self.displacement_x, self.displacement_z)
            ...
            if ( (not self.has_collided) and self.checkCollision(context)):
                ...
                with torch.autograd.profiler.profile(use_cuda=self.using_cuda) as prof:
                    ...
                    #print("coords two frames before:")
                    #print(self.circle_prev_coordinates_2)
                    #print("coords prev frame:")
            ...
            for vert in circle.data.vertices:
                ...
                # prendo la x e la z perche' in blender la z e' l'asse verticale, quello che in abaqus era la y
                # e moltiplico le coordinate per lo scaling parameter perche' prima le avevo divise
                self.circle_prev_coordinates_1.append([vert_global[0]*SCALING_PARAMETER, vert_global[2]*SCALING_PARAMETER])
    ...
    def execute(self, context):
        ...
        alpha = context.scene.simulation_properties.alpha
        ...
        # Posiz iniziale
        circle_origin_x = trajectory*math.sin(math.radians(alpha))
        circle_origin_z = trajectory*math.cos(math.radians(alpha))

        # Imposta posizione iniziale cerchio
        circle.location = (circle_origin_x, 0, circle_origin_z)

        # Calcola spostamenti
        self.displacement_x = -velocity*math.sin(math.radians(alpha))/FPS
        self.displacement_z = -velocity*math.cos(math.radians(alpha))/FPS
    ...
    def move_circle(self, context, displacement_x, displacement_z):
        circle = bpy.data.objects[CIRCLE_OBJECT_NAME]

        # Muovi cerchio
        circle.location.x += displacement_x
        circle.location.z += displacement_z
...
class MainPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # Slider angolo 
        col.prop(context.scene.simulation_properties, "alpha", text="Circle angle", slider=True)
        col.separator()
