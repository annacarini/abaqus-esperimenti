        ...
        if event.type == 'TIMER':
            ...
            if ( (not self.has_collided) and self.checkCollision(context)):
                ...
                with torch.autograd.profiler.profile(use_cuda=self.using_cuda) as prof:
                    ...
                    plate_edges = [[(edge.verts[0].index, edge.verts[1].index), edge] for edge in bm.edges]

                    edges_to_be_deleted = []
                    
                    for edge in plate_edges:
                        if (edge[0] in self.edges_to_remove):
                            edges_to_be_deleted.append(edge[1])

                    bmesh.ops.delete(bm, geom=edges_to_be_deleted, context='EDGES_FACES')
                    ...
                    plate_edges_initial = [(edge[0], edge[1]) for edge in plate.data["edges_initial"]]
                    ...
                    # vertici che hanno perso un edge:
                    ...
                    for edge in plate_all_edges:
                        edge_reformat = (edge[0], edge[1])
                        if ((edge[0] in plate_vertices_that_lost_an_edge and edge[1] in plate_vertices_that_lost_an_edge) and not (edge_reformat in self.edges_to_remove)):
                            #edges_to_create.append(edge)
                            # Creo il nuovo edge solo se non esiste:
                            if (not edge_reformat in plate_edges_initial):
                                print(edge_reformat)
