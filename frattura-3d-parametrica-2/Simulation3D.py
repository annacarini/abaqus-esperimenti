...
class Simulation3D():
    ...
    def __init__( self ):
        ...
        self.steel_ductile_damage = ((0.3945, 0.3333, 0),)
        self.steel_damage_evolution = ((0.3,),)


        # MESH
        self.circle_seed_size          = 0.7
        self.plate_seed_sides          = 1
    ...
    def runSimulation( self,
        ...
        material_plate.DuctileDamageInitiation( table = self.steel_ductile_damage ).DamageEvolution( type = abaqusConstants.DISPLACEMENT, table = self.steel_damage_evolution )
        ...
        # crea filtro per la velocita' verticale
        model.ButterworthFilter( name           = "Filter-Circle-V2",
        ...
        model.HistoryOutputRequest( name           = 'H-Output-Circle-V2', 
                                    ...
                                    frequency      = 200,
                                    filter         = "Filter-Circle-V2" )
    
    
        # crea filtro per la posizione verticale
        model.ButterworthFilter( name           = "Filter-Circle-COOR2",
                                # cutoffFrequency = frequency above which the filter attenuates at least half of the input signal
                                # => l'ho scelta a caso alta, da rivedere
                                    cutoffFrequency = 10000,
                                    operation       = abaqusConstants.MIN,
                                    limit           = - (3 * self.plate_height + CIRCLE_RADIUS),          # 4 volte lo spessore della lastra + il raggio
                                    halt            = True )      # per far fermare l'analisi quando il valore limit e' raggiunto


        # aggiungi una history output request per la coordinata verticale della palla, in modo da potergli applicare un filtro
        model.HistoryOutputRequest( name           = 'H-Output-Circle-COOR2', 
                                    createStepName = 'Step-1', 
                                    region         = RP_circle_set,
                                    variables      = ('COOR2',),        # velocita' verticale
                                    frequency      = 200,
                                    filter         = "Filter-Circle-COOR2" )
        ...
        # seed sugli edge verticali
        part_plate.seedEdgeBySize(edges = (edge_ne, edge_sw, edge_nw, edge_se),
                                  size = self.plate_seed_sides )
        ...
        log("quantita' nodi superficie palla:" + str(len(part_circle.sets['surface'].nodes)))
        '''
        ...
        # TEMP per fermare la simulazione
        ...
        if SAVECIRCLEVELOCITY:
            ...
            v2Data = region.historyOutputs['V2_FILTER-CIRCLE-V2'].data
