...
# Operatore per riprodurre l'animazione ed eseguire codice ad ogni frame
class OT_play_animationMyTimer(bpy.types.Operator):

    bl_idname = "wm.play_animation_my_timer"
    bl_label = "Modal Timer Operator"
    ...
    '''
    def modal2(self, selfObj, context):
            
        global totFrames

        totFrames += 1

        print(f"frame: {totFrames}")
        ...
        if (totFrames >= 40):
            ...
            # muovi cerchio
            selfObj.move_circle(context, selfObj.displacement_x, selfObj.displacement_y, selfObj.displacement_z)

            # se c'e' la collisione
            if ( (not selfObj.has_collided) and selfObj.checkCollision(context)):
                print("collisione!")
                selfObj.has_collided = True
    ...
    def modal(self, context, event):
        ...
        '''
        # cancella timer e ricrealo
        wm = context.window_manager
        wm.event_timer_remove(self._timer)
        new_timer_wait = (self.next_frame % 2) + 1      # 1 per numeri pari, 2 per numeri dispari
        self._timer = wm.event_timer_add(time_step=new_timer_wait/FPS, window=context.window)
        wm.modal_handler_add(self)
        '''

        print(f"frame {self.next_frame} eseguito dopo {time.time() - self.prev_frame_time}")
    ...
    def execute(self, context):
        ...
        # Avvia operatore
        #bpy.app.timers.register(functools.partial(self.modal2, self, context), first_interval=1/FPS)
        #return {'FINISHED'}
        wm = context.window_manager
        self._timer = wm.event_timer_add(time_step=1/FPS, window=context.window)
        wm.modal_handler_add(self)
...
class MainPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # Pulsante play 2
        row = layout.row()
        row.operator("wm.play_animation_my_timer", text="Play 2")
...
def register():
    ...
    bpy.utils.register_class(OT_play_animationMyTimer)      #****************
...
def unregister():
    ...
    bpy.utils.unregister_class(OT_play_animationMyTimer)      #****************
